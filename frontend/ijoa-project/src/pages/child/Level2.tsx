...
const Level2: React.FC = () => {
  ...
  const [currentModalLevel, setCurrentModalLevel] = useState(3);
  ...
  const renderModalContent = () => {
    const babyImage = currentModalLevel === 3 ? baby3 : baby4;
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      <img src={bglv2} alt="배경화면" className="w-full h-screen object-cover" />
      <img
        src={profile}
        alt="프로필이미지"
        className="absolute bottom-24 left-[calc(50%-60px)] transform -translate-x-1/2 w-[160px] z-10 rounded-3xl"
      />
      <img src={baby2} alt="애기2" className="absolute bottom-5 left-1/2 transform -translate-x-1/2 w-[300px]" />
          ...
          <div className="relative bg-[#FCFFEA] p-10 rounded-lg shadow-lg w-3/4 max-w-2xl text-center">
            ...
            <div className="flex items-center justify-center gap-x-8 mb-6">
              <button
                ...
                className={`${currentModalLevel !== 3 ? "visible" : "invisible"} w-20 h-20`}
              >
